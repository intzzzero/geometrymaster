...
import {
	findStarPeaks,
	calculateStarAngleConsistency,
	calculateStarSymmetry,
} from './star-helper-functions';
...
function calculateCircularity(points: Point[]): number {
	...
	const circumferenceRatio =
		Math.min(perimeter, theoreticalCircumference) /
		Math.max(perimeter, theoreticalCircumference);

	// 최종 원형도 = 원주율 정확도 중심 (85%), 반지름 일관성 (10%), 둘레 비율 (5%)
	return Math.min(
		1,
		piAccuracy * 0.85 + radiusConsistency * 0.1 + circumferenceRatio * 0.05
	);
}
...
function calculateCompleteness(points: Point[]): number {
	...
	const avgRadius =
		points.reduce((sum, point) => sum + distance(point, center), 0) /
		points.length;
...
export function scoreCircle(points: Point[]): ScoringResult {
	...
	if (accuracy < 0.05) {
		finalScore = Math.min(finalScore, 15.0); // 매우 낮은 정확도
	} else if (accuracy < 0.1) {
		finalScore = Math.min(finalScore, 30.0); // 낮은 정확도
	} else if (accuracy < 0.2) {
		finalScore = Math.min(finalScore, 50.0); // 보통 이하 정확도
	} else if (accuracy < 0.4) {
		finalScore = Math.min(finalScore, 70.0); // 보통 정확도
	} else if (accuracy < 0.6) {
		finalScore = Math.min(finalScore, 85.0); // 높은 정확도
	} else if (accuracy < 0.8) {
		finalScore = Math.min(finalScore, 95.0); // 매우 높은 정확도
	}
	...
	return {
		score: Number(Math.min(100.0, Math.max(0.0, finalScore)).toFixed(3)),
		feedback,
		details: {
...
export function scoreStar(points: Point[]): ScoringResult {
	...
	const peakScore =
		starPeaks.length === 5
			? 1
			: Math.max(0, 1 - Math.abs(starPeaks.length - 5) / 5);

	// 각 꼭짓점 간의 각도 간격 검증 (72도 간격)
	const angleConsistency = calculateStarAngleConsistency(
		starPeaks,
		points,
		center
	);
	...
	if (finalScore >= 99.0) {
		feedback =
			'Perfect 5-pointed star! Golden ratio and angle spacing are very accurate! ⭐️';
	} else if (finalScore >= 95.0) {
		feedback =
			'Excellent 5-pointed star! Ratio and angles are very accurate! 👏';
	} else if (finalScore >= 85.0) {
		...
		feedback =
			'Close to a star shape but needs improvement as a 5-pointed star.';
	} else {
...
export function scoreSquare(points: Point[]): ScoringResult {
	...
	const cornerScore =
		corners.length === 4
			? 1
			: Math.max(0, 1 - Math.abs(corners.length - 4) / 4);
	...
	const accuracy =
		...
		lengthBalance * 0.15 +
		squareRatio * 0.1; // 가로세로 비율 1:1 (10%)
	...
	if (rightAngleScore < 0.05 || cornerScore < 0.2) {
		rawScore = Math.min(rawScore, 20.0); // 기본 형태도 안 됨
	} else if (rightAngleScore < 0.1 || squareRatio < 0.3) {
		rawScore = Math.min(rawScore, 40.0); // 사각형이지만 정사각형과 거리 있음
	} else if (rightAngleScore < 0.25) {
		rawScore = Math.min(rawScore, 60.0); // 어느 정도 사각형 형태
	} else if (rightAngleScore < 0.4) {
		rawScore = Math.min(rawScore, 75.0); // 좋은 사각형
	} else if (rightAngleScore < 0.6) {
		rawScore = Math.min(rawScore, 88.0); // 매우 좋은 사각형
	} else if (rightAngleScore < 0.8 || squareRatio < 0.7) {
		rawScore = Math.min(rawScore, 95.0); // 거의 완벽한 사각형
	}
	...
	let feedback = '';
	if (finalScore >= 99.0) {
		feedback = 'Perfect square! All angles are very close to 90°! 🟦';
	} else if (finalScore >= 95.0) {
		feedback =
			'Excellent square! Right-angle accuracy and proportions are very high! 👏';
	} else if (finalScore >= 85.0) {
		feedback = 'Good square! Try drawing the corners a bit more precisely.';
	} else if (finalScore >= 70.0) {
		feedback = 'Decent square. Make the four right angles more accurate.';
	} else if (finalScore >= 50.0) {
		feedback =
			'Close to a square, but needs improvements to be a perfect square.';
	} else {
		feedback =
			'Try again! Draw a square with four right angles and equal sides.';
	}
...
// 직선으로부터의 편차 계산
function calculateLineDeviation(
	points: Point[],
	start: Point,
	end: Point
): number {
...
function calculateRightAngles(corners: Point[]): number {
	...
	if (angles.length === 4) {
		const totalAngleDegrees =
			angles.reduce((sum, angle) => sum + angle, 0) * (180 / Math.PI);
		const anglesSumAccuracy = Math.max(
			0,
			1 - Math.abs(totalAngleDegrees - 360) / 60
		);
...
function calculateLengthBalance(corners: Point[]): number {
	...
	const variance =
		sideLengths.reduce((sum, len) => sum + (len - avgLength) ** 2, 0) /
		sideLengths.length;
...
function calculateSquareRatio(corners: Point[]): number {
	...
	// 두 대각선의 길이가 비슷해야 함
	const diagonalRatio =
		Math.min(diagonal1, diagonal2) / Math.max(diagonal1, diagonal2);
...
export function scoreTriangle(points: Point[]): ScoringResult {
	...
	const cornerScore =
		corners.length === 3
			? 1
			: Math.max(0, 1 - Math.abs(corners.length - 3) / 3);
	...
	const accuracy =
		...
		equilateralScore * 0.4 + // 내각 60도 정확도가 핵심 (40%)
		symmetryScore * 0.05;
	...
	if (equilateralScore < 0.05 || cornerScore < 0.2) {
		rawScore = Math.min(rawScore, 18.0); // 기본 형태도 안 됨
	} else if (equilateralScore < 0.1) {
		rawScore = Math.min(rawScore, 35.0); // 삼각형이지만 정삼각형과 거리 있음
	} else if (equilateralScore < 0.25) {
		rawScore = Math.min(rawScore, 55.0); // 어느 정도 정삼각형 형태
	} else if (equilateralScore < 0.4) {
		rawScore = Math.min(rawScore, 75.0); // 좋은 정삼각형
	} else if (equilateralScore < 0.6) {
		rawScore = Math.min(rawScore, 88.0); // 매우 좋은 정삼각형
	} else if (equilateralScore < 0.8) {
		rawScore = Math.min(rawScore, 95.0); // 거의 완벽한 정삼각형
	}
	...
	if (finalScore >= 99.0) {
		feedback =
			'Perfect equilateral triangle! All angles are very close to 60°! 🔺';
	} else if (finalScore >= 95.0) {
		feedback =
			'Excellent equilateral triangle! Angle accuracy is very high! 👏';
	} else if (finalScore >= 85.0) {
		feedback = 'Good triangle! Try drawing the vertices a bit more precisely.';
	} else if (finalScore >= 70.0) {
		feedback = 'Decent triangle. Make the three angles closer to 60°.';
	} else if (finalScore >= 50.0) {
		feedback =
			'Close to a triangle shape, but needs improvement toward an equilateral triangle.';
	} else {
		feedback =
			'Try again! Draw an equilateral triangle with three equal sides.';
	}
...
function findTriangleCorners(points: Point[]): Point[] {
	...
	for (let i = 3; i < points.length - 3; i++) {
		...
		if (angleDiff > threshold) {
			...
			const isDuplicate = corners.some(
				(corner) => distance(corner, points[i]) < 30
			);
			if (!isDuplicate) {
				corners.push(points[i]);
			}
...
function getCornerSharpness(points: Point[], corner: Point): number {
	...
	if (cornerIdx === -1 || cornerIdx < 3 || cornerIdx >= points.length - 3)
		return 0;
...
// 삼각형 직선성 계산
function calculateTriangleStraightness(
	points: Point[],
	corners: Point[]
): number {
...
// 삼각형 변에 속하는 점들 추출
function getTriangleSegmentPoints(
	points: Point[],
	start: Point,
	end: Point
): Point[] {
...
function calculateEquilateralAngles(corners: Point[]): number {
	...
	if (angles.length === 3) {
		...
		const anglesSumAccuracy = Math.max(
			0,
			1 - Math.abs(totalAngleDegrees - 180) / 30
		);
